...
const symbols: Record<Op, string> = {
  ...
  const : "$0", ifelse : "($0 ? $1 : $2)", neg : "(-$0)", mod : "($0%$1)", sub: "($0-$1)"}
...
export const compile = (x:Schedule[]) => {
  ...
  let raster = (x:Schedule) :string => {

    if (seen.has(x)) return seen.get(x);
    let c = (x.srcs.length? x.srcs.map(raster) : ([(x as {arg:number}).arg.toString()])) .reduce((p, c, i) => p.replaceAll(`$${i}`, c), symbols[x.op])
    if (x.uses > 1) {
